...
mod tests {
    ...
    fn random_plain_proof_tests(#[case] num: usize) {
        ...
        let result = Psc::<4, 32, 3>::verify_membership(element, &witness, &set_commitment);
        ...
        let result = Psc::<4, 32, 3>::verify_membership(&fake_element, &witness, &set_commitment);
        assert_eq!(result.is_ok(), true);
    ...
    fn random_eh_halo_tests(#[case] num: usize) {
        ...
        let set_commitment = psc.get_commitment();
        let result = Psc::<LN, M, LE>::eh_verify_halo(&proof, &set_commitment, &param, pk.get_vk());
        assert_eq!(result.is_ok(), true);
        assert_eq!(result.unwrap(), true);
